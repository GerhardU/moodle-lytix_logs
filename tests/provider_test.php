 ...
 * @coversDefaultClass \lytix_logs\privacy\provider
 */
final class provider_test extends provider_testcase {
    ...
    public static function generate_data_for_user($course, $user) {
        ...
        $record->targetid = rand(1, 1000);
        $record->timestamp = rand(1, 1000);
        $record->widget = "widget";
        $record->meta = "meta";
        $DB->insert_record('lytix_logs_logs', $record);
        ...
        $record->contextid = 50;
        $record->target = "target";
        $record->duration = rand(1, 1000);
        $record->day = rand(1, 1000);
        $DB->insert_record('lytix_logs_aggregated_logs', $record);
    }
    ...
    public function test_get_users_in_context_non_user_context(): void {
        ...
        $userlist = new userlist($context, 'lyitx_logs');
    ...
    public function test_delte_user_over_contextlist(): void {
        ...
        $this->assertTrue($this->privacy_delete_context_user_sub_test($user, 'lytix_logs_logs'));
        $this->assertTrue($this->privacy_delete_context_user_sub_test($user, 'lytix_logs_aggregated_logs'));
    }
    ...
    public function test_metadata(): void {
        ...
        $this->assertTrue($this->privacy_get_metadata_sub_test('lytix_logs_logs'));
        $this->assertTrue($this->privacy_get_metadata_sub_test('lytix_logs_aggregated_logs'));
    }
    ...
    public static function privacy_delte_users_sub_test(\stdClass $user): bool {
        ...
        $ul = new approved_userlist(\context_system::instance(), "lytix/logs", [$user->id]);
        $privacy::delete_data_for_users($ul);
        return empty($DB->get_records_sql(
            "SELECT * FROM {lytix_logs_logs} WHERE userid = :userid", [ "userid" => $user->id])) &&
            empty($DB->get_records_sql(
                "SELECT * FROM {lytix_logs_aggregated_logs} WHERE userid = :userid", [ "userid" => $user->id]));
    }
    ...
    public static function privacy_get_users_sub_test(\stdClass $user): bool {
        ...
        $context = new contextlist();
        $context->add_system_context();
        $ul = new \core_privacy\local\request\userlist($context->get_contexts()[0], "lytix/logs");
    ...
    public static function privacy_delete_context_sub_test(): bool {
        ...
        return empty($DB->get_records_sql("SELECT * FROM {lytix_logs_logs}")) &&
            empty($DB->get_records_sql("SELECT * FROM {lytix_logs_aggregated_logs}"));
    }
    ...
    public static function privacy_get_metadata_sub_test(string $component): bool {
        ...
        return empty($collection->get_collection()) ? false :
            (count($collection->get_collection()[0]->get_privacy_fields()) >= 6 ? true : false);
    }
